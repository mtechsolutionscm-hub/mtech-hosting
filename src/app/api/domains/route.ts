...
export async function GET() {
  ...
  const domains = await prisma.domain.findMany({ where: { website: { organization: { memberships: { some: { userId: user.id } } } } }, include: { website: true, certificates: true, dnsRecords: true }, orderBy: { createdAt: "desc" } });
...
export async function POST(request: Request) {
  ...
  const verificationToken = `mtech-domain-${randomBytes(18).toString("hex")}`;
  const domain = await prisma.$transaction(async tx => {
    ...
    return tx.domain.create({ data: { websiteId: parsed.data.websiteId, hostname: parsed.data.hostname, isPrimary: parsed.data.isPrimary ?? false, verificationToken } });
  });
  await audit("CREATE", "Domain", domain.id, user.id, { hostname: domain.hostname });
  return NextResponse.json({ domain, verification: { type: "TXT", name: `_mtech-verification.${domain.hostname}`, value: verificationToken } }, { status: 201 });
}
